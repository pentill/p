				...
				if (body.startsWith('#gtts ')) {
				   ...
				   client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					try {
						...
						if (args.lenght !== 2) return client.sendMessage(from, 'Maksimal 2 kata\n\nContoh: #phub kata1 kata2', msgType.text, {quoted: mek})
						if (q.lenght > 20) return client.sendMessage(from, 'Teks terlalu panjang, maksimak 20 huruf cuy', msgType.text, {quoted: mek})
					const buffer = await getBuffer(`https://api.vhtear.com/pornlogo?text1=' + args[0] + '&text2=' + args[1]  + '&apikey=' + ${vkey}`)
						client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, 'Sepertinya sedang error!', msgType.text, {quoted: mek})
					}
					...
					try {
						const teks = encodeURIComponent(body.slice(8))
						const anu = await fetchJson(`https://arugaz.herokuapp.com/api/stalk?username=${teks}`, {method: 'get'})
