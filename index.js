			...
			if (type == 'conversation') {
				...
				if (body.startsWith('#gtts ')) {
				   ...
				   client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					const teks = encodeURIComponent(body.slice(8))
						if (!teks) return client.sendMessage(from, 'Input teks yang ingin di tulis', msgType.text, {quoted: mek})
					const buffer = await getBuffer(`https://api.vhtear.com/glitchtext?text1=${teks}&apikey=${vkey}`)
					client.sendMessage(from, buffer, msgType.image, {quoted: mek})
				...
				} else if (body.startsWith('#jodoh ')) {
					try {
						const teks = encodeURIComponent(body.slice(7))
						if (!teks) return client.sendMessage(from, '_Perintah doang yang diketik, yang mau dicari apa?_', msgType.text, {quoted: mek})
						const anu = await fetchJson(`https://arugaz.herokuapp.com/api/jodohku?nama=arga&pasangan=${teks}`, {method: 'get'})
						const hasil = `*Nama* : ${anu.nama}\n*Pasangan* : ${anu.pasangan}\n*Positif* : ${anu.positif}\n*Negatif* : ${anu.negatif}`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
					}
				} else if (body.startsWith('#zodiak ')) {
					try {
						const teks = encodeURIComponent(body.slice(8))
						if (!teks) return client.sendMessage(from, '_Perintah doang yang diketik, yang mau dicari apa?_', msgType.text, {quoted: mek})
						const anu = await fetchJson(`https://arugaz.herokuapp.com/api/getzodiak?nama=aruga&tgl-bln-thn=${teks}`, {method: 'get'})
						const hasil = `*Nama* : ${anu.nama}\n*Usia* : ${anu.usia}\n*Ultah* : ${anu.ultah}\n*Lahir* : ${anu.lahir}\n*Zodiak* : ${anu.zodiak}`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
