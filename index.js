				...
				if (body.startsWith('#gtts ')) {
				   ...
				   client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					const teks = encodeURIComponent(body.slice(5))
						...
						const anu = await fetchJson(`https://https://nekos.life/api/v2/img/tits`, {method: 'get'})
						const buffer = await getBuffer(anu.url)
					    client.sendMessage(from, buffer, msgType.image, {quoted: mek, caption})
					...
					try {
						const teks = encodeURIComponent(body.slice(6))
						const anu = await fetchJson(`https://nekos.life/api/v2/img/tits`, {method: 'get'})
						const buffer = await getBuffer(anu.url)
						client.sendMessage(from, buffer, msgType.image, {quoted: mek, caption: '>///<'})
					} catch (e) {
						console.log(`Error : ${e}`)
					...
					try {
						...
						const buffer = await getBuffer(anu.images)
						const hasil = `${anu.result} ${anu.references} ${anu.source}`
						client.sendMessage(from, buffer, msgType.text, {quoted: mek, caption: `${hasil}`})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
					}
