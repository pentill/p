			...
			if (type == 'conversation') {
				...
				if (body.startsWith('#gtts ')) {
				   ...
				   client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					const teks = encodeURIComponent(body.slice(8))
						if (!teks) return client.sendMessage(from, 'Input teks yang ingin di tulis', msgType.text, {quoted: mek})
					const buffer = await getBuffer(`https://api.vhtear.com/glitchtext?text1=${teks}&text2=▱&apikey=${vkey}`)
					client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					try {
						const teks = encodeURIComponent(body.slice(8))
						if (!teks) return client.sendMessage(from, '_Perintah doang yang diketik, yang mau dicari apa?_', msgType.text, {quoted: mek})
						const anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/stalk?username=${teks}&apiKey=${apiKey}`, {method: 'get'})
						const hasil = `*Username* : ${anu.Username}\n*Biodata* : ${anu.Biodata}\n*Jumlah Followers* : ${anu.Jumlah_Followers}\n*Jumlah Following* : ${anu.Jumlah_Following}\n*Profil Pic* : ${anu.Profile_pic}`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
					...
					try {
						const teks = encodeURIComponent(body.slice(10))
						if (!teks) return client.sendMessage(from, '_Perintah doang yang diketik, yang mau dicari apa?_', msgType.text, {quoted: mek})
						const anu = await fetchJson(`https://arugaz.herokuapp.com/api/artinama?nama=${teks}`, {method: 'get'})
					...
					try {
						...
						if (!teks) return client.sendMessage(from, '_Perintah doang yang diketik, yang mau dicari apa?_', msgType.text, {quoted: mek})
						const anu = await fetchJson(`https://arugaz.herokuapp.com/api/jadwalshalat?daerah=${teks}`, {method: 'get'})
						const hasil = `*Daerah ${teks}*\n*Dzuhur* : ${anu.result[0].Dzuhur}\n*Ashar* : ${anu.result[0].Ashar}\n*Maghrib* : ${anu.result[0].Maghrib}\n*Isya* : ${anu.result[0].Isya}\n*Subuh* : ${anu.result[0].Subuh}\n*Dhuha* : ${anu.result[0].Dhuha}\n*Imsyak* : ${anu.result[0].Imsyak}`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
