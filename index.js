	...
	client.on('message-new', async (mek) => {
		try {
			...
			if (type == 'conversation') {
				...
				} else if (body.startsWith('#url2img ')) {
					try {
						const teks = encodeURIComponent(body.slice(9))
						if (!teks) return client.sendMessage(from, '_Perintah doang yang diketik, yang mau dicari apa?_', msgType.text, {quoted: mek})
						const anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/url2image?tipe=desktop&url=${url}&apiKey=${apiKey}`, {method: 'get'})
						const hasil = `${anu.result} ${anu.filesize}`
						client.sendMessage(from, hasil, msgType.image, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
					}
