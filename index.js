			...
			if (type == 'conversation') {
				...
				if (body.startsWith('#gtts ')) {
				   ...
				   client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					try {
						const teks = encodeURIComponent(body.slice(7))
						const pasangan = nama.split(/\&/)
						const anu = await fetchJson(`https://arugaz.herokuapp.com/api/jodohku?nama=${pasangan[0]}&pasangan=${pasangan[1]}`, {method: 'get'})
						const buffer = await getBuffer(anu.gambar)
						const hasil = `${anu.nama}\n${anu.pasangan}\n${anu.positif}\n${anu.negatif}`
						client.sendMessage(from, buffer, msgType.images, {quoted: mek, caption: `${hasil}`})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
					}
