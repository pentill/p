			...
			if (type == 'conversation') {
				...
				if (body.startsWith('#gtts ')) {
				   ...
				   client.sendMessage(from, buffer, msgType.image, {quoted: mek})
					...
					if (!isOwner) return client.sendMessage(from, '_Owner only_', msgType.text, {quoted: mek})
					if (args[0].startsWith('08')) return client.sendMessage(from, '$Gunakan kode negara_', msgType.text, {quoted: mek})
					try {
						const teks = body.slice(7)
						client.blockUser (`${body.slice(7)}@c.us`, "add")
						client.sendMessage(from, `Perintah Diterima, memblokir ${body.slice(7)}@c.us`, msgType.text, {quoted: mek})
						} catch (e) {
							console.log(`Error : ${e}`)
							}
					...
					if (args[0].startsWith('08')) return client.sendMessage(from, '$Gunakan kode negara_', msgType.text, {quoted: mek})
					try {
						const teks = body.slice(9)
						const unblock = client.blockUser (`${body.slice(9)}@c.us`, "remove")
						client.sendMessage(from, `Perintah diterima, unblock ${body.slice(9)}`, msgType.text, {quoted: mek})
						} catch (e) {
							console.log(`Error : ${e}`)
							}
					...
					try {
						...
						const anu = await fetchJson(`https://st4rz.herokuapp.com/api/simsimi?kata=${teks}`, {method: 'get'})
						const hasil = `${anu.result}\n\n*SimSimi*`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
					...
					try {
						const teks = encodeURIComponent(body.slice(6))
						const anu = await fetchJson(`https://arugaz.herokuapp.com/bapakfont?kata=${teks}`, {method: 'get'})
						const hasil = `${anu.result}`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
				...
				} else if (body.startsWith('#jadwalshalat ')) {
					try {
						const teks = encodeURIComponent(body.slice(14))
						const anu = await fetchJson(`https://tobz-api.herokuapp.com/api/jadwalshalat?q=${teks}`, {method: 'get'})
						const hasil = `*Dzuhur* : ${anu.result.dzuhur}\n*Ashar* : ${anu.result.ashar}\n*Maghrib* : ${anu.result.maghrib}\n*Isya* : ${anu.result.isha}\n*Subuh* : ${anu.result.subuh}\n*Imsak* : ${anu.result.imsak}\n*Sunrise* : ${anu.result.sunrise}\n*Sunset* : ${anu.result.sunset}\n*Midnight* : ${anu.result.midnight}`
						client.sendMessage(from, hasil, msgType.text, {quoted: mek})
					} catch (e) {
						console.log(`Error : ${e}`)
						client.sendMessage(from, '_Tidak dapat menemukan_', msgType.text, {quoted: mek})
